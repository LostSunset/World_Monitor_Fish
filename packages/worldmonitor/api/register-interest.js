...
async function sendConfirmationEmail(email, referralCode) {
  ...
  try {
    ...
    if (!resendRes.ok) {
      ...
      console.log(`[register-interest] Email sent to ${maskedEmail}`);
    }
  } catch (err) {
...
export default async function handler(req) {
  ...
  // Redis-based distributed rate limit (effective on edge/serverless)
  const redisBlock = await checkRateLimit(req, cors);
  if (redisBlock) return redisBlock;

  // In-memory fallback (best-effort within a single isolate)
