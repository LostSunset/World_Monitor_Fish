...
<title>${og.title}</title>
  ...
  // NOTE: User-Agent-based detection is defense-in-depth only. The UA header is
  // trivially spoofable, so this must NOT be the sole security control. Backend
  // endpoints must enforce their own authentication and rate limiting independently.
  if (BOT_UA.test(ua)) {
